...
from .core import compute_quality_flags, missing_table, summarize_dataset, is_unique_identificators, zeros_table
...
class QualityFlagsResponse(BaseModel):
    """
    Ответ эндпоинта флагов модели качества данных 
    """
    
    flags: dict[str, bool] | None = Field (
        default=None, 
        description="Булевы флаги с подроностями."
    )
...
async def quality_from_csv(file: UploadFile = File(...)) -> QualityResponse:
    ...
    (summarize_dataset, missing_table, compute_quality_flags, is_unique_identificators, zeros_table)
    ...
    unique_df = is_unique_identificators(df)
    zeros_df = zeros_table(df)
    flags_all = compute_quality_flags(summary, missing_df, unique_df, zeros_df)
...
@app.post(
    ...
    tags=["quality"],
    summary="Список флагов по CSV-файлу, после использования EDA-ядра",
)
async def quality_flags_from_csv(file: UploadFile = File(...)) -> QualityFlagsResponse:
    """
    Эндпоинт, который принимает CSV-файл, запускает EDA-ядро
    (summarize_dataset, missing_table, compute_quality_flags, is_unique_identificators, zeros_table), 
    и возвращает буллевый словарь флагов в формате JSON, а не всю оценку датасета. 
    """ 
    
    if file.content_type not in ("text/csv", "application/vnd.ms-excel", "application/octet-stream"):
        raise HTTPException(status_code=400, detail="На вход ожидается CSV-файл (content-type text/csv).")
    
    try: 
        df = pd.read_csv(file.file)
    except Exception as e:
        raise HTTPException(status_code=400, detail=f"Не удалось открыть CSV-файл: {e}")
    ...
    flags_all = compute_quality_flags(summary_df, missing_df, unique_df, zeros_df) #создаем флаги
    
    
    
    flags_bool: dict[str, bool] = {
        key: bool(value)
        for key, value in flags_all.items()  # Берем именно булевы флаги 
        if isinstance(value, bool)
    }
