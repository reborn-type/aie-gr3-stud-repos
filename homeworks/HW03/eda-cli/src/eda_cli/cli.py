...
def report(
    ...
    title: str = typer.Option("EDA-отчет", help="Загаловок для отчета."),
    min_missing_share: float = typer.Option(0.5, help="Минимальный порог доли пропусков.")
) -> None:
    """
    ...
    # 2. Качество в целом
    quality_flags = compute_quality_flags(summary, missing_df, unique_df, zeros_df, min_missing_share)
    ...
    with md_path.open("w", encoding="utf-8") as f:
        ...
        f.write(f"- Доля уникальных идентификаторов по колонке: **{quality_flags['unique_id_percent']:.2%}**\n")
        f.write(f"- Средняя доля нулей: **{quality_flags['mean_zeros_share']:.2%}**\n")
        ...
        if high_missing.empty:
            f.write(f"Коллонок с долей пропусков выше {min_missing_share:.0%} нет или датасет пуст.\n\n")
        else:
